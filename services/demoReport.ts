...
const tampered: AnalysisReport = {
  ...
  modules: [
    ...
    },
    {
      ...
      score: 10,
      narrative:
        "The income figure is set in a different font family and weight than the rest of the document — a regular-weight Trebuchet MS/Verdana rather than the bold Arial used everywhere else on the page — with a visible vertical baseline offset of roughly three to four pixels relative to where the surrounding text sits inside the field box (≈x=110–1130px, y=656–731px). The right edge of the figure, '96,500/-', also crowds the box's printed border more tightly than the left edge, consistent with a shorter replacement string typeset to right-align against the same rule line as the original figure. The edges of the pasted text are also softer than the crisp anti-aliasing on the rest of the page, consistent with a lower-generation re-render being composited on top.\n\nThe certificate serial shows the same pattern at smaller scale: only the last three digits, '219', sitting inside the ≈x=294–554px, y=442–469px strip, carry a slightly heavier monospace weight than the 'MD/NVP/IC/2025/048' segment immediately to their left, which matches the surrounding document's weight exactly — the edit is isolated to the trailing digits, not the full serial.",
      checks: [
