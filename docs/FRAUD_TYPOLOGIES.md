# Fraud Typologies — What Pramaan Is Actually Looking For

This is the working catalog behind Pramaan's forensic reasoning: for each document class in
scope, how fraudsters actually forge or tamper it, which of those forgeries leave a
**detectable signal**, and which Pramaan output field that signal should land in. It exists so
...
| Pramaan field | What it means | Who/what produces it |
|---|---|---|
...
| `externalChecksNeeded` | Anything that needs a live registry/database Pramaan does not query today | Claude is instructed to list these, never fabricate the result |
...
- **Wrong-category "help"**: a tehsildar-office tout or cyber café operator files a real
  ...
  Pramaan) to catch, because the artifact is authentic; only the underlying facts are false. A
...
- For PDF submissions: `PDF modified` timestamp significantly after `PDF creation` timestamp
  (Pramaan's own deterministic flag, see `api/_core.ts` — "PDF edited after creation"), or a
...
e-District/Lok Seva Kendra portal — Pramaan does not and cannot query this live today).
...
- **Genuine marksheet, fabricated backing exam**: the Vyapam pattern — the underlying "exam" was
  ...
  above, no pixel-level forensic tool (Pramaan included) can detect this from the document alone;
...
arithmetic check Pramaan can actually run with certainty, no external source needed) ·
...
- Aadhaar's QR code (present on the standard PDF/print) that doesn't visually correspond to a
  ...
  though Pramaan's current pipeline does not decode QR payloads; a mismatched/garbled/absent QR
...
- Aadhaar number failing the public Verhoeff checksum structure (a purely mathematical
  well-formedness check on the 12-digit number — Pramaan can validate this deterministically
...
`externalChecksNeeded` (this is the single most important category to be explicit about: Pramaan
...
- **Selective statement periods**: submit only the months that look favorable, omitting overdrafts
  ...
  Pramaan can flag as a limitation rather than "prove."
...
Pramaan's strongest, least-external-dependency check for this document class; IFSC-bank name
match; salary-slip-vs-statement cross-check) · `technicalSignals` (producer string, edit-after-
creation) · `externalChecksNeeded` (confirming the statement against the bank directly — Pramaan
...
**Cross-field / logical (this is where Pramaan's reasoning layer earns its keep)**
...
- Invoice value inconsistent with typical market pricing for the stated goods at the stated
  quantity — flagged as a plausibility observation, not a certainty, since Pramaan has no live
  pricing database.
...
the GST Network's public GSTIN-search API, which Pramaan does not currently call; confirming
actual movement of goods needs e-way bill/logistics data Pramaan does not have access to).
...
- Signature on the BG inconsistent in style/pressure with the bank officer's known signing
  pattern where a comparison specimen exists (rarely available to Pramaan without external data).
...
instead of the bank's own published contact channel. Pramaan's honest limitation here is explicit
...
- **Duplicate-listing fraud**: the same property photos and "ownership" documents reused across
  ...
  cross-listing comparison, which is outside what a single-document analysis (Pramaan's current
...
portal — Pramaan does not query this; confirming stamp-paper authenticity requires the state
...
modus operandi is directly relevant to Pramaan's design: fraudsters **misuse real people's
...
2023. This is a directly-on-point precedent for Pramaan's tender-document use case (§6 above) and
...
rather than the bank's independently-obtained official channel — precisely the limitation Pramaan
...
1. **Lead with the arithmetic/logical checks, not the visual ones.** Line-items-sum-to-total,
   ...
   execution-date are the checks Pramaan can assert with near-certainty and zero external
...
2. **The `externalChecksNeeded` list is not a weakness to hide — it's the credibility feature.**
   ...
   trusted a self-reported contact channel instead of an independent one. Pramaan explicitly
...
3. **Identity-document fraud (Aadhaar/PAN/GST) and invoice/ITC fraud are the same underlying
   ...
   then issue fake invoices. Pramaan's per-document, cross-field reasoning is one layer of defense
